...
class MatrixFactorization(nn.Module):
    ...
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        self.emb_to_str = False
        try:
            self.emb_to_str = config['emb_to_str']
    ...
    def user_encoder(self,users,all_users=None):
        # print("user max:", users.max(), users.min())
        # if not self.emb_to_str:
        if self.emb_to_str:
            return self.u_emb_str[users.cpu()]
        else:
            return self.user_embedding(users)
    def item_encoder(self,items,all_items=None):
        # print("items max:", items.max(), items.min())
        if self.emb_to_str:
            return self.i_emb_str[items.cpu()]
        else:
            return self.item_embedding(items)
    
    def computer(self): # does not need to compute user reprensentation, directly taking the embedding as user/item representations
        if self.emb_to_str:
            if self.u_emb_str is None:
                self.u_emb_str = self.user_embedding.weight.data.cpu().numpy()
                self.i_emb_str = self.item_embedding.weight.data.cpu().numpy()
                if self.umap_dim <= self.u_emb_str.shape[0]:
                    reducer = umap.UMAP(random_state=42,n_components=self.umap_dim)
                    self.u_emb_str = reducer.fit_transform(self.u_emb_str)
                    reducer = umap.UMAP(random_state=42,n_components=self.umap_dim)
                    self.i_emb_str = reducer.fit_transform(self.i_emb_str)
                self.u_emb_str = [" ".join( [one_en for one_en in one_line.astype(str)] ) for one_line in self.u_emb_str ]
                self.i_emb_str = [" ".join( [one_en for one_en in one_line.astype(str)] ) for one_line in self.i_emb_str ]
                self.u_emb_str = np.array(self.u_emb_str)
                self.i_emb_str = np.array(self.i_emb_str)
    ...
    def do_rec(self, user, cliked):
        #do recommendation according to the user id and exclude clicked items
...
#CoLLM, where the target item id and user id are projected to LLM input space by a same MLP(following is a linear layer)
...
"""
hashGNN implemented by Yang Zhang
"""
...
class hashGNN(nn.Module):
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        self.config = config
        self.padding_index = 0
        self.rec_model = MatrixFactorization(config=config)  #base MF, just two embedding matrixes
        ...
        try:
            ...
            self.use_hash = True
        try: 
            self.direct_id = config['direct_id']
        except:
            self.direct_id = False
        print("########hashGNN, use hash?  ", self.use_hash)
    ...
    def computer(self):
        if self.training or not self.eval_done_computer: # training or evaluation at first step
            ...
            if self.use_hash:
                ...
                else:  # directly represent the id as str and includes to prompts
                    hash_u = list(range(u_emb.shape[0]))
                    hash_i = list(range(i_emb.shape[0]))
                    hash_u = [str(x) for x in hash_u]
                    hash_i = [str(x) for x in hash_i]
                    self.hash_u = np.array(hash_u)
                    self.hash_i = np.array(hash_i)
                if self.training:
                    self.eval_done_computer = False
                ...
                # self.hash_u = [str(x) for x in u_z]
                def array_to_str(ss):
                    m = []
                    for x in ss:
                        x_s = str(x.item())
                        x_s = x_s[0:min(len(x_s),7)]
                        m.append(x_s)
                    return " ".join(m)
                self.hash_i = np.array([array_to_str(x) for x in i_z])
                self.hash_u = np.array([array_to_str(x) for x in u_z])
                if self.training:
                    self.eval_done_computer = False
                else:                                # evaluation will not repeatly computed the embedding if no traning is involved
                    if not self.eval_done_computer:  
                        self.eval_done_computer  = True
        return self.hash_u, self.hash_i
