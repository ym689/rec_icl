...
class AmazonOOData(RecBaseDataset):
    def __init__(self, text_processor=None, ann_paths=None,incontext_learning=False,incontext_sample_len=0):
        super().__init__()
        ...
        # 0905change
        if 'sessionItems' in self.annotation.columns or 'his' in self.annotation.columns:
            if incontext_learning:
                used_columns = ['uids','iids','titles','hiss', 'his_titles','labels','model']
                renamed_columns = ['UserID','TargetItemID','TargetItemTitle', 'InteractedItemIDs', 'InteractedItemTitles','label','model']
                if 'not_cold' in self.annotation.columns:
                    used_columns.append("not_cold")
                    renamed_columns.append("prompt_flag")
                    self.prompt_flag = True
        ...
        if self.use_his:
            if incontext_learning:
                max_length_ = 0
                for i in self.annotation['InteractedItemIDs'].values:
                    for x in i: 
                        max_length_ = max(max_length_, len(x))
                self.max_lenght = min(max_length_, 10) # average: only 50; 0915: 15 
            else:
    ...
    def __getitem__(self, index):
        ...
        if self.use_his:
            if self.incontext_learning:
                a = ann['InteractedItemIDs']
                InteractedNum = []
                InteractedNum = [len(i) for i in a]
                b = []
                for i in range(len(a)):
                    if a[i][0] == 0:
                        InteractedNum[i] -= 1
                for i in range(len(a)):
                    if len(a[i]) < self.max_lenght:
                        temp = [0]*(self.max_lenght-len(a[i]))
                        temp.extend(a[i])
                        b.append(temp)
                    elif len(a[i]) > self.max_lenght:
                        temp = a[i][-self.max_lenght:]
                        InteractedNum[i] = self.max_lenght
                        b.append(temp)
                    else:
                        temp = a[i]
                        b.append(temp)
                ...
                # b = a
                # InteractedNum = len(a) # for in context learning, we delete some thing useless
                #0905change
                one_sample = {
                    "UserID": torch.tensor(ann['UserID']),
                    "InteractedItemIDs_pad": np.array(b),
                    # "InteractedItemIDs_pad": b,
                    # "InteractedItemIDs": ann['InteractedItemIDs'],
                    "InteractedItemTitles": '###'.join([convert_title_list_v2(ann['InteractedItemTitles'][i][-InteractedNum[i]:]) for i in range(len(ann['InteractedItemTitles']))]),
                    # "InteractedItemTitles": [convert_title_list_v2(ann['InteractedItemTitles'][i]) for i in range(len(ann['InteractedItemTitles']))],
                    "TargetItemID": torch.tensor(ann["TargetItemID"]),
                    # "TargetItemTitle": ["\""+ann["TargetItemTitle"][i].strip(' ')+"\"" for i in range(len(ann["TargetItemTitle"]))],
                    "TargetItemTitle": '###'.join(["\""+ann["TargetItemTitle"][i].strip(' ')+"\"" for i in range(len(ann["TargetItemTitle"]))]),
                    "InteractedNum": np.array(InteractedNum),
                    "label": np.array(ann['label']),
                    "model": ann['model']
                }
                if self.prompt_flag:
                    one_sample['prompt_flag'] = ann['prompt_flag']
