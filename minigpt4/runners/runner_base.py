...
class RunnerBase:
    ...
    def __init__(self, cfg, task, model, datasets, job_id):
        ...
        self._model = model
        self._device=None
        self.incontext_learning = cfg.model_cfg.incontext_learning
        self.incontext_sample_len = cfg.model_cfg.incontext_sample_len
        self.shuffle_train_data = cfg.model_cfg.shuffle_train_data
    ...
    def train(self):
        ...
        if not self.evaluate_only:# with training
            for cur_epoch in range(self.start_epoch, self.max_epoch):
                ...
                # if not self.evaluate_only and self.model_to_betrained():  ##to train tallrec, which without ID, must remove this condition judge
        ...
        def _create_loader(dataset, num_workers, bsz, is_train, collate_fn):
            ...
            if isinstance(dataset, ChainDataset) or isinstance(
                ...
                loader = DataLoader(
                    ...
                    pin_memory=True,
                    sampler=sampler,
                    shuffle=sampler is None and is_train and (self.shuffle_train_data if is_train else True),
                    collate_fn=collate_fn,
                    drop_last=True if is_train else False,
                )
    ...
    def log_stats(self, stats, split_name):
        if isinstance(stats, dict):
            ...
            if "valid_uauc" in log_stats and isinstance(log_stats["valid_uauc"],tuple):
                print(log_stats["valid_uauc"])
                log_stats["valid_uauc"] = list(log_stats["valid_uauc"])
                for i in range(len(log_stats["valid_uauc"][1])):
                    log_stats["valid_uauc"][1][i] = int(log_stats["valid_uauc"][1][i])
                log_stats["valid_uauc"][2] = list(log_stats["valid_uauc"][2])
