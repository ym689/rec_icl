...
if __name__=='__main__':
    ...
    lr_=[1e-3] #1e-2
    dw_ = [1e-5,1e-6,1e-7]
    embedding_size_ = [16,32, 64, 128, 256, 512]
    # embedding_size_ = [256]
    save_path_prefix = "/data/yanming/datasets_13_17/processed_ds/0_6/mf_models/patience_200_"
    try:
        f = open("/data/yanming/datasets_13_17/processed_ds/0_6/"+"mf_patience_200"+".log",'a+')
    except:
        f = open("/data/yanming/datasets_13_17/processed_ds/0_6/"+"mf_patience_200"+".log",'w+')
    # f=None
    for lr in lr_:
        for wd in dw_:
            for embedding_size in embedding_size_:
                ...
                save_path = save_path_prefix + f"lr_{lr}_wd_{wd}_ebd_{embedding_size}"
